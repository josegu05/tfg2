...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=.7, width=.7\textwidth]{transformer}
...
\begin{align*}
   \frac{N_p}{N_s} = \frac{V_p}{V_s} &= \frac{V_p}{V_s} = n \\
   Z_p &= V_p \cdot I_p \\
   Z_s &= V_s \cdot I_s \\
\end{align*}
\begin{equation}
...
Se tiene que $Z_p = h_{ob}^{-1}$ y que la impedancia de la antena es aproximadamente $Z_s = R_{rad} = \SI{1}{\kilo\ohm}$
Por lo que, se usa la ecuaci\'on \ref{eq:transformer} para obtener el ratio de vueltas óptimo siendo $$ n = \sqrt{\frac{Z_p}{Z_s}} = 10 $$
